...
{
    RCLCPP_INFO(node->get_logger(), "[superodometry] read parameter");
    std::string calib_file;
    // calib_file = readParam<std::string>(node, "calib_file");
    calib_file = node->declare_parameter("calibration_file", std::string(""));
    RCLCPP_INFO(node->get_logger(), "[superodometry] calib_file: %s", calib_file.c_str());
    ...
    else
    {
        ...
        // RCLCPP_INFO(node->get_logger(),  "T_i_l Extrinsic : \n"
        //           << T_i_l.matrix();
        // RCLCPP_INFO(node->get_logger(),  "T_l_i Extrinsic : \n"
        //           << T_l_i.matrix();
    }
...
bool readGlobalparam(rclcpp::Node::SharedPtr node)
{
    ...
    node->declare_parameter<std::string>("PROJECT_NAME", "superodometry");
