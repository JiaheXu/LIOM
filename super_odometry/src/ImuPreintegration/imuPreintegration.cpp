...
namespace superodom {
    ...
    void imuPreintegration::initInterface() {
        ...
        subLaserOdometry = this->create_subscription<nav_msgs::msg::Odometry>(
            ProjectName+"/aft_mapped_to_init_incremental", 5,
        ...
        pubImuOdometry2 = this->create_publisher<nav_msgs::msg::Odometry>(
            ProjectName+"/integrated_to_init", 10);
...
# if 0
    ...
    sensor_msgs::msg::Imu imuPreintegration::imuConverter(const sensor_msgs::msg::Imu &imu_in) {
        ...
        q_extrinsic=Eigen::Quaterniond(imu_laser_R);
    ...
    {
        // RCLCPP_INFO(this->get_logger(),"imuCB from IMU_TOPIC");
        std::lock_guard<std::mutex> lock(mBuf);
        // RCLCPP_ERROR(this->get_logger(),"imuCB ax %lf ay %lf az %lf", imu_raw->linear_acceleration.x, imu_raw->linear_acceleration.y, imu_raw->linear_acceleration.z);
       ...
       // publish tf2
        tf2::Transform tCur;
        tf2::fromMsg(odometry2.pose.pose, tCur);
        // tCur.setRotation(tCur.getRotation().normalize());
        // RCLCPP_WARN_STREAM(this->get_logger(), "tfT" << tCur.getOrigin().x() << ", " << tCur.getOrigin().y() << ", " << tCur.getOrigin().z());
        // RCLCPP_WARN_STREAM(this->get_logger(), "tfR" << tCur.getRotation().x() << ", " << tCur.getRotation().y() << ", "<< tCur.getRotation().z() << ", " << tCur.getRotation().w());

        geometry_msgs::msg::TransformStamped odom_2_baselink;
        odom_2_baselink.header.stamp = thisImu.header.stamp;
        odom_2_baselink.header.frame_id = WORLD_FRAME;
        // odom_2_baselink.child_frame_id_ = SENSOR_FRAME;
