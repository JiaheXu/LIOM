...
{
    //RCLCPP_WARN(this->get_logger(), "release marginlizationinfo");
...
void MarginalizationInfo::marginalize()
{
    ...
    //RCLCPP_DEBUG(this->get_logger(), "marginalization, pos: %d, m: %d, n: %d, size: %d", pos, m, n, (int)parameter_block_idx.size());
    ...
    RCLCPP_INFO(this->get_logger(),"summing up costs %f ms", t_summing.toc());
    */
    //multi thread
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        if (ret != 0) {
            RCLCPP_WARN(this->get_logger(), "pthread_create error");
            ROS_BREAK();
        }
    }
    ...
    //RCLCPP_DEBUG(this->get_logger(), "thread summing up costs %f ms", t_thread_summing.toc());
    //RCLCPP_INFO(this->get_logger(),"A diff %f , b diff %f ", (A - tmp_A).sum(), (b - tmp_b).sum());
