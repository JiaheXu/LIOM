...
void readParameters(rclcpp::Node::SharedPtr node)
{
    ...
    if (FISHEYE == 1)
        FISHEYE_MASK = VINS_FOLDER_PATH + "fisheye_mask_640x480.png";
    CAM_NAMES.push_back(config_file);
    LOG(INFO)<<"FISHEYE_MASK:"<<FISHEYE_MASK;
    ...
    if (ESTIMATE_EXTRINSIC == 2) {
        RCLCPP_WARN(this->get_logger(), "have no prior about extrinsic param, calibrate extrinsic param");
        ...
        if (ESTIMATE_EXTRINSIC == 1) {
            RCLCPP_WARN(this->get_logger(), " Optimize extrinsic param around initial guess!");
            EX_CALIB_RESULT_PATH = OUTPUT_PATH + "/extrinsic_parameter.csv";
        }
        if (ESTIMATE_EXTRINSIC == 0)
            RCLCPP_WARN(this->get_logger(), " fix extrinsic param ");
