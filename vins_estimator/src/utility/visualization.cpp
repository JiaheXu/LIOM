...
{
  pub_latest_odometry = n.advertise<nav_msgs::msg::Odometry>("imu_propagate", 1000);
  pub_path = n.advertise<nav_msgs::msg::Path>("path", 1000);
  pub_relo_path = n.advertise<nav_msgs::msg::Path>("relocalization_path", 1000);
  pub_odometry = n.advertise<nav_msgs::msg::Odometry>("odometry", 1000);
  pub_loam_odometry = n.advertise<nav_msgs::msg::Odometry>("lidar/odometry", 1000);
  pub_point_cloud = n.advertise<sensor_msgs::msgPointCloud>("point_cloud", 1000);
  pub_margin_cloud =
      n.advertise<sensor_msgs::msgPointCloud>("history_cloud", 1000);
  pub_lidar_cloud = n.advertise<sensor_msgs::msg::PointCloud2>("lidar_cloud", 1000);

  pub_key_poses = n.advertise<visualization_msgs::msg::Marker>("key_poses", 1000);
  pub_camera_pose = n.advertise<nav_msgs::msg::Odometry>("camera_pose", 1000);
  pub_map_projection_image =
      n.advertise<sensor_msgs::msgImage>("depth_image", 1000);
  pub_camera_pose_visual =
      n.advertise<visualization_msgs::msg::MarkerArray>("camera_pose_visual", 1000);
  pub_slidewindow_camera_pose =
      n.advertise<visualization_msgs::msg::MarkerArray>("slidewindow_pose", 1000);
  pub_keyframe_pose = n.advertise<nav_msgs::msg::Odometry>("keyframe_pose", 1000);
  pub_keyframe_point =
      n.advertise<sensor_msgs::msgPointCloud>("keyframe_point", 1000);
  pub_extrinsic = n.advertise<nav_msgs::msg::Odometry>("extrinsic", 1000);
  pub_relo_relative_pose =
      n.advertise<nav_msgs::msg::Odometry>("relo_relative_pose", 1000);
  pub_delta_p = n.advertise<std_msgs::msg::Float64>("delta_p", 1000);
  pub_delta_v = n.advertise<std_msgs::msg::Float64>("delta_v", 1000);
  pub_delta_t = n.advertise<std_msgs::msg::Float64>("delta_t", 1000);
  pub_vloamPath = n.advertise<nav_msgs::msg::Path>("vio_loam_path", 100);
                       ...
                       const std_msgs::msg::Header &header)
{
  ...
  {
    static tf2::Transform t_odom_world = tf2::Transform(tf::createQuaternionFromRPY(0, 0, M_PI), tf2::Vector3(0, 0, 0));
    if (header.stamp.toSec() - last_align_time > 1.0)
    {
      try
      {
        tf::StampedTransform trans_odom_baselink;
        listener.lookupTransform(LIO_World_Frame, LIO_Laser_Frame, rclcpp::Time(0), trans_odom_baselink);
  ...
  {
    tf2::Transform t_static = tf2::Transform(tf::createQuaternionFromRPY(0, 0, 0), tf2::Vector3(0, 0, 0));
...
tf2::Transform transformConversion(const tf::StampedTransform& t)
{
    ...
    return tf2::Transform(tf::createQuaternionFromRPY(rollCur, pitchCur, yawCur), tf2::Vector3(xCur, yCur, zCur));;
}
...
void printStatistics(const Estimator &estimator, double t)
{
  ...
  {
    // RCLCPP_DEBUG(this->get_logger(), "calibration result for camera %d", i);
  ...
  sum_of_time += t;
  sum_of_calculation++;
  RCLCPP_DEBUG(this->get_logger(), "vo solver costs: %f ms", t);
  RCLCPP_DEBUG(this->get_logger(), "average of time %f ms", sum_of_time / sum_of_calculation);

  sum_of_path += (estimator.Ps[WINDOW_SIZE] - last_path).norm();
  last_path = estimator.Ps[WINDOW_SIZE];
  RCLCPP_DEBUG(this->get_logger(), "sum of path %f", sum_of_path);
  //    if (ESTIMATE_TD)
  //        RCLCPP_INFO(this->get_logger(),"td %f", estimator.td);
}
...
void pubOdometry(const Estimator &estimator, const std_msgs::msg::Header &header)
{
  ...
  if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
  {
    ...
    geometry_msgs::msg::PoseStamped visualLOAMPose;
    ...
    RCLCPP_DEBUG(this->get_logger(), "\033[1;32m----> delta_p_v:%f,%f,%f|%f,%f,%f \033[0m",delta_p.x(),delta_p.y(),delta_p.z(),delta_v.x(),delta_v.y(),delta_v.z());
    RCLCPP_DEBUG(this->get_logger(), "norm   :%f|%f",delta_p.norm(),delta_v.norm());
    RCLCPP_DEBUG(this->get_logger(), "delta_t  :%f",dt);
    std_msgs::msg::Float64 rosp;
    std_msgs::msg::Float64 rosv;
    std_msgs::msg::Float64 rost;
    ...
    ofstream foutC(VINS_RESULT_PATH, std::ios::app);
...
void pubPointCloud(const Estimator &estimator, const std_msgs::msg::Header &header)
{
  ...
  // pub margined potin
  sensor_msgs::msgPointCloud margin_cloud;
                              ...
                              const std_msgs::msg::Header &header)
{
  if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
  {
    ...
    sensor_msgs::msg::PointCloud2 rosmsg_output;
    ...
    for (auto iter = pc_in_new.begin(); iter != pc_in_new.end(); ++iter)
    {
      ...
      //判断激光点云 是否在图像平面上
      if (iter->z > 0 and frontend::is_in_image(uv, frontend::Boundary,
                                                1))
      { // && iter->z < 5.0
    ...
    sensor_msgs::msgImagePtr depthImgMsg =
...
void pubKeyframe(const Estimator &estimator)
{
          ...
          Estimator::MarginalizationFlag::MARGIN_OLD)
  {
    ...
    sensor_msgs::msgPointCloud point_cloud;
    ...
    for (auto &landmark : estimator.f_manager.KeyPointLandmarks)
    {
              ...
              WINDOW_SIZE - 2)) != landmark.second.obs.end())
      {
...

        {
          ...
          sensor_msgs::msgChannelFloat32 p_2d;
...
{
  nav_msgs::msg::Odometry odometry;
  odometry.header.stamp = rclcpp::Time(estimator.relo_frame_stamp*1e9);
  odometry.header.frame_id = VINS_World_Frame;
