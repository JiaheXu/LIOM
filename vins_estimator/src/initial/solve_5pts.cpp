...
bool
MotionEstimator::solveRelativeRT(const Eigen::aligned_vector<pair<Eigen::Vector3d, Eigen::Vector3d>> &corres,
                                 ...
                                 Eigen::Vector3d &Translation)
{
    if (corres.size() >= 15) {
        ...
        if (inlier_cnt > 12) {
            RCLCPP_ERROR(this->get_logger(),"----------5points-----------");
            RCLCPP_ERROR(this->get_logger(),ll.size());
            RCLCPP_ERROR(this->get_logger(),inlier_cnt);
            RCLCPP_ERROR(this->get_logger(),Rotation);
            RCLCPP_ERROR(this->get_logger(),Translation);
            return true;
        }
        else
            return false;
...
bool
MotionEstimator::solveRelativeRT_PNP(const Eigen::aligned_vector<pair<Eigen::Vector3d, Eigen::Vector3d>> &corres,
                                     Eigen::Matrix3d &Rotation,
                                     Eigen::Vector3d &Translation)
{
    ...
    //
    //        residualV = (tp2 - tp1);
    //        RCLCPP_ERROR(this->get_logger(),residualV.transpose());
    //    }
